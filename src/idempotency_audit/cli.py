...
from idempotency_audit import __version__
from idempotency_audit.core import (
    audit_records,
    read_records,
    render_json,
    render_markdown,
...
from idempotency_audit.rules import PROJECT_NAME


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog='idempotency-audit',
        description='Check mutating API specs for idempotency and retry safety.',
    )
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=Path,
        help='API endpoint notes, OpenAPI snippets, or review text',
    )
    parser.add_argument(
        "--format",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"idempotency-audit: error: {exc}", file=sys.stderr)
        return 1
